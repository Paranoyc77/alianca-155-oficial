...
import { Search, ExternalLink, Users, Megaphone, Globe, Sparkles, Loader2, Lock, UserPlus, Music, Play, Pause, Send } from "lucide-react";
import { Link } from "wouter";
import { toast } from "sonner";

export default function PublicHome() {
  const [activeTab, setActiveTab] = useState<"all" | "grupo" | "canal" | "site" | "recrutamento">("all");
      ...
      <section className="px-6 py-14 md:py-20 text-center max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 pt-4">
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-bold text-xs md:text-sm transition flex items-center gap-2 ${activeTab === "all" ? "bg-[#8b5cf6] text-white shadow-[0_4px_20px_rgba(139,92,246,0.3)]" : "bg-[#111111]/80 hover:bg-[#1a1a1a] text-[#969696] border border-[#262626]"}`}
          >
            Todos ({counts.all})
          </button>
          <button
            onClick={() => setActiveTab("grupo")}
            className={`px-5 py-2.5 rounded-xl font-bold text-xs md:text-sm transition flex items-center gap-2 ${activeTab === "grupo" ? "bg-[#8b5cf6] text-white shadow-[0_4px_20px_rgba(139,92,246,0.3)]" : "bg-[#111111]/80 hover:bg-[#1a1a1a] text-[#969696] border border-[#262626]"}`}
          >
            <Users className="w-4 h-4" /> Grupos ({counts.grupo})
          </button>
          <button
            onClick={() => setActiveTab("canal")}
            className={`px-5 py-2.5 rounded-xl font-bold text-xs md:text-sm transition flex items-center gap-2 ${activeTab === "canal" ? "bg-[#8b5cf6] text-white shadow-[0_4px_20px_rgba(139,92,246,0.3)]" : "bg-[#111111]/80 hover:bg-[#1a1a1a] text-[#969696] border border-[#262626]"}`}
          >
            <Megaphone className="w-4 h-4" /> Canais ({counts.canal})
          </button>
          <button
            onClick={() => setActiveTab("site")}
            className={`px-5 py-2.5 rounded-xl font-bold text-xs md:text-sm transition flex items-center gap-2 ${activeTab === "site" ? "bg-[#8b5cf6] text-white shadow-[0_4px_20px_rgba(139,92,246,0.3)]" : "bg-[#111111]/80 hover:bg-[#1a1a1a] text-[#969696] border border-[#262626]"}`}
          >
            <Globe className="w-4 h-4" /> Sites ({counts.site})
          </button>
          <button
            onClick={() => setActiveTab("recrutamento")}
            className={`px-5 py-2.5 rounded-xl font-bold text-xs md:text-sm transition flex items-center gap-2 ${activeTab === "recrutamento" ? "bg-gradient-to-br from-[#8b5cf6] to-[#5b21b6] text-white shadow-[0_4px_20px_rgba(139,92,246,0.4)]" : "bg-[#111111]/80 hover:bg-[#1a1a1a] text-[#bca9ff] border border-[#8b5cf6]/40"}`}
          >
